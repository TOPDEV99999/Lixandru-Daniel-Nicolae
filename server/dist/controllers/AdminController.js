...
class AdminController {
    ...
    contactRepository;
    userRepository;
    constructor(visitorRepository, meetingRepository, contactRepository, userRepository) {
    ...
    getRecentActivity(meetings) {
        const activity = [];
        const today = new Date().toISOString().split('T')[0];
        const yesterday = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString().split('T')[0];
        const todayMeetings = meetings.filter(m => new Date(m.createdAt).toISOString().split('T')[0] === today);
        const yesterdayMeetings = meetings.filter(m => new Date(m.createdAt).toISOString().split('T')[0] === yesterday);
    ...
    calculateGrowthRate(visitors, meetings, messages) {
    ...
    calculateMeetingConversionRate(meetings) {
