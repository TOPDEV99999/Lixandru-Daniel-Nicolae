...
const meetingValidation_1 = require("../validation/meetingValidation");
...
class MeetingController {
    meetingRepository;
    visitorRepository;
    constructor(meetingRepository, visitorRepository) {
        this.meetingRepository = meetingRepository;
        this.visitorRepository = visitorRepository;
    }
    async submitMeeting(req, res) {
        try {
            // Validate input
            const validationResult = meetingValidation_1.meetingRequestSchema.safeParse(req.body);
            ...
            const { customerName, email, company, meetingTopic, requestedDate, requestedTime, notes, visitorIp, browser, country, userId } = validationResult.data;
            ...
            const recentCount = recentMeetings.filter(meeting => new Date(meeting.createdAt) > oneHourAgo).length;
            if (recentCount >= 3) {
            ...
            const conflictingMeetings = existingMeetings.filter(meeting => meeting.requestedTime === requestedTime &&
    ...
    async getMeetingRequest(req, res) {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Not authenticated' });
            }
    ...
    async respondToMeeting(req, res) {
        try {
            ...
            const { id } = req.params;
            // Validate input
            const validationResult = meetingValidation_1.meetingResponseSchema.safeParse(req.body);
            if (!validationResult.success) {
                return res.status(400).json({
                    error: 'Validation failed',
                    details: validationResult.error.errors.map(err => ({
    ...
    async updateMeetingRequest(req, res) {
        try {
            ...
            // Validate input
            const validationResult = meetingValidation_1.meetingUpdateSchema.safeParse(req.body);
            if (!validationResult.success) {
                return res.status(400).json({
                    error: 'Validation failed',
                    details: validationResult.error.errors.map(err => ({
    ...
    async deleteMeetingRequest(req, res) {
        try {
            if (!req.user || req.user.role !== 'admin') {
                return res.status(403).json({ error: 'Admin access required' });
            }
